...
function isRetryableDbError(err: unknown): boolean {
  ...
  // 40001 = PG serialization failure / recovery conflict
  // P2034 = Prisma transaction write conflict / deadlock
...
async function retry<T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.warn(`[precompute] ${label} attempt ${attempt} failed (db conflict), retrying in ${wait}ms...`);
...
export async function precomputeAll(): Promise<{ durationMs: number; keys: number }> {
  ...
  try {
    // Phase 1: Dashboard + dates + tags + rankings (all independent)
    ...
    // Phase 2: Potential analysis — all platforms × all days in parallel
    console.log("[precompute] Phase 2: Potential analysis (all platforms)...");
    await runBatch(
