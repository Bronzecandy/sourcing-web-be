...
2. **`buildLibraryRequests(context, libraryEntries, rubric)`** — **sau** merge rubric:
   ...
   - Ghi `be/data/libraries/pending-additions.json` qua `persistLibraryRequestsToFile` (dedupe type+label).
...
- Đừng ghi `persistLibraryRequestsToFile` **trước** `mergeRubricFromLlm` (mất điểm AI trên pending).
